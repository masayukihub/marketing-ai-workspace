...
- 用户要求 10 个以上商品或提供目录型历史截图时，使用 `autumn-catalog-v1` 的 10–18 SKU 目录结构；首屏只选 4–6 个产品，商品区保持清楚的双列层级，附加 Coupon、新品、品牌与 LINE 模块按当前证据启用。
- 产品角色决定视觉权重，已选历史 Recipe 决定大卡、横向图文、网格等具体形式；不要把所有邮件强制重排成同一种白底商品卡。每个商品只保留一个主要利益点。
- 模块之间留白、模块内部聚合；不得通过缩小文字解决信息过载。

### 5. 写自然日语

- 用户要求多案时，在交付 HTML 中放入 3 组可切换的 Subject / Preheader，不能只写在聊天回复里。
...
- 用户要求可编辑时输出 `editable-review.html`，支持修改内容、价格、购买链接和可选模块，并导出 JSON、原生邮件 HTML 与可继续编辑的 HTML。修改后重新渲染才可更新最终 PNG；不能冒称图片会自动同步。
- 价格表未实际读到时，记录具体读取缺口并保留价格位；不能用历史折扣或官网当前价替代指定秋促价格。有真实当前草稿来源的数字可在内部稿中展示并标明 `参考価格・要確認`，不升级审批状态。
