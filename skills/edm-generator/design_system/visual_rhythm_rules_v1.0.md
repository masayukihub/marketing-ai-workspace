# EDM Visual Generator — Visual Rhythm Rules v1.0

状态：**Frozen / Phase 3 Closed**  
基线：`standards/edm_design_rules_v1.0.yaml`  
Human Decision：`research/phase3_design_system_human_decisions.csv`（6/6 Rhythm = Keep）  
...
2026-08-19 Human Review 将以下六组节奏全部标记为 `Keep`：
...
这些规则已冻结为 Soft/Advisory 组合约束。它们不能覆盖 Design Standard v1.0 Hard Rules，且不能把 Experimental Rule 转为 Blocking Constraint。
