...
用户有实际商品图参考、商业渲染目标或整套风格否定时，先读取 [商业画面制作契约](../amazon-listing-creative/references/commercial-visual-production.md)。继承用户范围、故事和有效批准，同时明确每张的画面类型、像素级参考观察、视觉证据和实际生产能力。不能把 Pattern/Template 名称当成画面类型已经匹配。
...
三层分开是来源与编辑要求，不是固定白栏加产品抠图的成片样式。先匹配镜头、光影、接触和动作位置，文字随后服从画面。功能效果可作为不含产品结构的独立环境/效果素材；它不能替代真实部件、内部结构或机械证据。目标为商业场景/机理画面时，先制作少量代表图并与已实际查看的参考比对；现有授权内自主修订，不自动填写人工批准。若实际工具只能输出信息图，明确能力不匹配，不批量扩大错误类型。
...
另外分别记录画面类型是否匹配、产品是否融入场景、效果是否解释作用过程。文件数量、解码、哈希与模板结构分不能替代这些实际看图结论。商业参考任务使用创意模块的 `visual-check` 绑定代表图观察与实际文件；检查器不替代视觉判断。
