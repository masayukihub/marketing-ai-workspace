...
进入生产或处理“出图不好看”时，读取 [creative-production.md](references/creative-production.md)。先查看已有成图与参考，给当前资产补齐镜头、场景、证明对象和分层合成 Brief；默认先做一个候选，看实际输出后局部修改。
...
明确本张采用官方素材合成、无产品场景生成、证明图解、UI合成还是排版。实际调用相应工具并记录输入与输出；固定模板或占位图不能冒充场景生成。文案长度、版式编号、品牌开关都不能自动证明画面质量。
...
逐张与整组审核都要绑定当前输出版本。必须打开实际文件，记录具体观察及需要修改的资产；未观察的品牌适配、真实感、模板感和手机可读性保持未评估。任何输出被替换后，重新检查相关单图和整组，模型审核不代表用户批准。
