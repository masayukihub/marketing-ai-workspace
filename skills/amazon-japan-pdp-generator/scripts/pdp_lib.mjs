...
export async function writeMarkdown(data, outputDir) {
  ...
  const qaRows = [
    ...(data.images || []).map((x) => [x.id, x.userQuestion, `Layout ${x.layoutId}`, x.round1Review, x.round2Qa?.status, x.creativeReviewResult?.status || "Needs Visual Review", x.creativeReviewResult?.fiveSecondMessage || "未观察", x.mobileSafeArea || x.wireframe?.mobileRule, x.productBodyAiGenerated === false ? "产品本体非AI" : "产品来源门槛未通过", x.visualProductionStatus]),
    ...(data.aplusModules || []).map((x) => [x.id, x.storyRole || x.purpose, x.templateType, x.round1Review, x.round2Qa?.status, x.creativeReviewResult?.status || "Needs Visual Review", x.creativeReviewResult?.fiveSecondMessage || "未观察", x.mobileConsideration, (x.units || []).every((u) => u.productBodyAiGenerated === false) ? "产品本体非AI" : "产品来源门槛未通过", x.visualProductionStatus]),
  ];
  const qaReport = `# Visual QA\n\n## 结论\n\n- 自动检查仅覆盖文案长度、版式元数据与产品来源约束；它不能判断构图、真实感、5秒理解或实际手机可读性。\n- Final 路径是兼容输出路径，其中的 JPEG 仍可能待审。查看精确成图并记录 creativeReview 后，才可通过创意审核；原有人工、Claim、素材和发布门槛继续生效。\n\n${markdownTable(["Visual", "购买问题 / 作用", "Layout / Template", "Round 1审阅", "文字/版式检查", "成图审核", "观察到的5秒信息", "Mobile", "产品", "制作状态"], qaRows)}\n`;
