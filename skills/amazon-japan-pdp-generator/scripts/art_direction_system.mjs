...
export function buildArtDirectionSpec(spec) {
  ...
  const score = {
    ...
    product_focus:92,
    visual_proof:88,
    scene_authenticity:84,
    camera_variety:cameraWarnings.length?82:94,
    product_scale_rhythm:scaleWarnings.length?82:91,
    material_lighting_fit:91,
    jp_lifestyle_fit:90,
    brand_fit:92,
    production_feasibility:78,
  };
  score.overall = semantic.status === "PASS" ? Math.round(Object.values(score).reduce((a,b)=>a+b,0)/Object.keys(score).length) : 0;
  return {
    schema_version:"1.0",
    ...
    quality:{
      score,
      score_scope:"Art Direction brief readiness, not final rendered-image quality",
      semantic_relevance:{score:semantic.status === "PASS" ? 100 : 0,status:semantic.status,hard_gate:true},
      jp_lifestyle_fit:{score:90,status:"BRIEF_READY_ASSET_MISSING",issues:["Installed-door proof is not yet usage-approved","Compatibility geometry is not yet engineering-approved","Entryway scenario production remains missing"]},
      current_baseline_score:46,
      recommended_art_direction_score:score.overall,
...
function contactSheet(spec, art){
  const body=`<nav><a href="#gallery">Gallery</a><a href="#aplus">A+</a><a href="#score">Score</a><span>Spec ${esc(art.source_spec_sha256.slice(0,12))}</span></nav><main><section class="hero"><span>ART DIRECTION CONTACT SHEET</span><h1>${esc(spec.product.name)}<br>Production Brief</h1><p>Story / Claim / Reference / Visual Role / Primitive frozen. Placeholder is review-only.</p><div class="kpis"><div><b>${art.validation.record_count}</b>visual units</div><div><b>${art.p0_asset_gaps.length}</b>P0 gaps</div><div><b>${art.quality.score.overall}</b>brief score</div><div><b>${art.quality.jp_lifestyle_fit.score}</b>JP fit</div></div></section><section id="gallery"><h1>Gallery · Decision Acceleration</h1>${art.gallery.map(galleryCard).join("")}</section><section id="aplus"><h1>A+ · Understanding / Trust / Ownership</h1>${art.aplus_modules.map(moduleCard).join("")}</section><section class="score" id="score"><h1>Art Direction Quality</h1>${Object.entries(art.quality.score).map(([k,v])=>`<div><span>${esc(k.replaceAll("_"," "))}</span><meter min="0" max="100" value="${v}"></meter><b>${v}</b></div>`).join("")}<p>评分只衡量Brief可执行性，不代表缺失素材已经生产。</p></section></main>`;
