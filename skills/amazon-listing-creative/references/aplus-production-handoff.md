...
在现有 Asset Ledger/Run Manifest 中分别记录 Gallery、A+ 模块、Slide 和 Mobile 的进度，按实际成功文件更新，不等整批完成后才知道 EBC 是否存在。采用该 Runtime 已支持的局部生产接口，不发明不存在的 CLI 参数。
...
3. 已装配到已有 Amazon Fidelity/Review 页面，而非孤立图片目录；
4. 实际浏览器验证桌面、390px、每个轮播页和必需文字；本地预览不能冒充 Seller Central 最终验证。

HTML 不存在、A+ 未挂载、只显示第一张 Slide 或手机端漏文案时，分别报告问题，不得用“图片文件存在”覆盖。

只允许通过获准的本地预览方式运行浏览器；不要绕过安全限制。工具无法访问时标记 Browser QA 未完成。

## 8. 完成与发布分开报告

输出一份简短进度：Gallery 文件 x/y；A+ 模块 x/y；Slide x/y；Mobile x/y；Native Copy 状态；Preview 挂载状态；实际 Browser QA；Publish Gate。
