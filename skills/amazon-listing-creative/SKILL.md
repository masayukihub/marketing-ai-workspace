...
1. **Listing Set**：规划或重构完整 Listing 图片组。沿用当前已批准资产集合与方向；从零开始时按购买任务确定图数，先给一个推荐方向，有实质分歧时增加一个对照。
...
按用户指定数量和模式探索。只有明确要求 9 宫格/广泛探索时才生成 9 或 10 个方向；单张生产与局部修改不重新发散。每个探索方向必须改变故事、构图、视觉母题、用户体验或信息表达机制；只换颜色、背景或机位不算新方向。

优先覆盖与产品相关的尺度感、真实使用瞬间、产品美感、类别差异、证据呈现和异议消除。不要为了凑数使用不适合产品的模板。

### 6. 组织图片角色

以下是从零规划时的九图示例，不是默认数量或固定槽位。已有资产集合保留 ID、数量和顺序；具体 Amazon 合规细节必须另行验证：
...
只有用户要求实际图片时才调用图片生成或编辑能力。生产默认先完成一张候选；正式产品本体来自官方素材，图像模型只制作无产品场景层，日语文案与精确 UI 后期叠加。明确的 9 宫格任务逐张制作后再组合；不要一次生成九格。

实际生成后打开导出文件，逐张检查产品结构、SKU、手部与场景物理关系、透视/光影、文字、手机端可读性和当前图组节奏。记录具体观察与局部修复，绑定输出版本。无法确认的视觉细节标记为 `Needs Manual QA`。

### 9. 评分并推荐

按需要评估：CTR Potential、Instant Understanding、Differentiation、Brand Premium、Amazon Suitability、Evidence Strength 和 Product Accuracy。多方向探索时才推荐 Top 3；单张制作给出观察、风险和具体修复。说明判断依据，未观察项保留未评估。
...
没有实际观察或测量时不填分数；CTR Potential 等只能是待验证假设。用具体问题与证据决定修订，不因总分过线自动放行。产品准确性或事实合规不通过时，直接标记 `Blocked` 或 `Pending Verification`。
